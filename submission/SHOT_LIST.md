...
| 0:51–0:58 | Local data and privacy | Show JSON backup or the mobile Share sheet. State that native data is app-private and encrypted sync remains unavailable until a separate relay is published and tested |
...
- Do not stage a public sync success until the relay is public and a real two-device native test passes.
- Do not show guessed GitHub URLs. The current checkout has no remote and no published Release.
