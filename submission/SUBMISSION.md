...
- Windows／Linux 使用 Electron。Windows 本機已驗證可產生未簽章 x64 NSIS 預覽安裝檔，可能觸發 SmartScreen；Linux AppImage／`.deb` 的 Actions 建置流程已就緒。
- Android／iOS 使用 Capacitor。Android 最低為 Android 8.0／API 26、target API 36；Actions 可產生可側載的 debug-signed APK，但尚未發布，且穩定覆蓋更新仍需要私有正式簽章金鑰。
- iOS 目前只有原始碼，仍須在 macOS 使用 Xcode、Apple 簽章與合適的發布管道；沒有可直接安裝的 iOS 檔案。

GitHub Releases workflow 已能替 EXE、APK、AppImage、`.deb` 統一命名，產生 SHA-256 與 `availability.json`。Pages workflow 會在 Release 發布成功後，把同一份 GitHub Release 安裝檔顯示成網站下載按鈕；EXE 不會另外維護一份無法追蹤的副本。但目前 checkout 沒有 Git remote，`gh` 憑證也已失效，因此 **尚未真的發布 GitHub Release 或 Pages 網站**，下載清單仍為空。投稿與影片不得預先宣稱尚未發布的檔案已可用。
...
- GitHub Actions 原生封裝、GitHub Pages、固定 release asset 名稱、SHA-256 與下載 manifest；尚待連接實際 repository 後發布

## 連結

- 預覽網址：**連接 repository 後填入 `https://OWNER.github.io/REPOSITORY/`**
- 原始碼：**投稿前補上真正的公開 GitHub repository URL**
- 展示影片：`submission/afterglow-demo.mp4`（上傳後補上公開影片 URL）
- 下載連結：**目前尚無 GitHub Release；只在真實 artifact 發布並驗證後加入**

## 投稿前阻塞事項

- [ ] 建立或指定公開 GitHub repository、修復 `gh` 授權並設定 remote
- [ ] 將 GitHub Pages 的 Source 設為 GitHub Actions，發布後在無痕／登出瀏覽器確認網址可開啟
- [ ] 發布版本化 GitHub Release，實際下載並安裝 EXE、AppImage、`.deb`、APK
- [ ] 確認 Pages workflow 已自動產生 Release 下載清單，逐一驗證網站按鈕與 SHA-256
